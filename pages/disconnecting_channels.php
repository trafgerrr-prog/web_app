				...
				<table class="ui compact celled definition table">
					<thead>
						<tr>
							...
							<th>Создано</th>
							<th>Изменено</th>
							<th>Номера каналов</th>
							<th>Время разрыва связи</th>
							<th>Время восстановления связи</th>
							<th>Номер первого запроса</th>
							<th>Номер запроса провайдера</th>
							<th>Примечание</th>
						</tr>
					</thead>
					<tbody>
...
$query_select = "SELECT * 
FROM public.disconnecting_channels
ORDER BY id ASC ";
...
for ($i = 0; $i <= pg_num_rows($result) - 1; $i++) {
	...
	$ids_channel = $data["ids_channel"];
	$break_time = $data["break_time"];
	$recovery_time = $data["recovery_time"];
	$first_request_number = $data["first_request_number"];
	$provider_request_number = $data["provider_request_number"];
	$note = $data["note"];
	...
	echo
			...
			'<td>' . $break_time . '</td>' .
			'<td>' . $recovery_time . '</td>' .
			'<td>' . $first_request_number . '</td>' .
			'<td>' . $provider_request_number . '</td>' .
			'<td>' . $note . '</td>'
	;
}
