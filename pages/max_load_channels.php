...
<html>
	<head>
		...
		<title>Максимальная загрузка каналов</title>
	</head>
...
for ($i = 0; $i <= pg_num_rows($result) - 1; $i++) {
	...
	$incoming_speed = $data["incoming_speed"];
	$outgoing_speed = $data["outgoing_speed"];
	...
	echo
			...
			'<td>' . $updated_at . '</td>' .
			'<td>' . $id_channel . '</td>' .
			'<td>' . $incoming_speed . '</td>' .
			'<td>' . $outgoing_speed . '</td>'
	;
}
